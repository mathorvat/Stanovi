...
exports.update_all = function(req, res, next) {
 ...
 connection.query('SELECT url FROM stan where is_visible like 1 AND is_active like 1 AND lat NOT like "NEMA";', function(err, rows, fields)   
  {
    if (err) throw err; 
    
    for (i = 0; i < rows.length; i++) {
        console.log("Updated: ", rows[i].url);        
        get_oglas(rows[i].url , 'update');       
    }     
...
function get_oglas(url, action){
  axios.get(url).then((res) => {
    ...
    */   
    ...
    //if($('h3.ClassifiedDetailUnavailableNotice-title').text().includes("Ovaj oglas je")){
      /*
    if($('BlockStandard BlockStandard--alpha ClassifiedDetailUnavailableNotice')){
      is_active = 0;
    }    
    */
   ...
   // if(is_active == 1){      
      ...
      //var cijena_kn = $('.ClassifiedDetailSummary-priceDomestic').text();
      var cijena_eu = $('.ClassifiedDetailSummary-priceDomestic').text().split(" ~ ")[1].trim();
      //var cijena_eu = $('.ClassifiedDetailSummary-priceFOreignc').text();
...
function umetanje(stan){
  ...
  var sql = `SELECT * FROM stan where sifra_oglasa = "${stan.sifra_oglasa}"`;  
